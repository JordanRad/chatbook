...
// 			if strings.Contains(r.URL.Path, "login") || strings.Contains(r.URL.Path, "register") || strings.Contains(r.URL.Path, "info/profile") {
// 				h.ServeHTTP(w, r)
// 				return
// 			}
	...
	return func(h http.Handler) http.Handler {
		return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			...
			if isProtectedRoute {
				...
				ctx := context.Background()
				fmt.Println(u.Email)
				dbu, err := us.GetAuthUserByEmail(ctx, u.Email)
				if err != nil {
					fmt.Println(err.Error())
					w.WriteHeader(http.StatusInternalServerError)
					w.Write([]byte("500 - Middleware error, cannot retrieve user from registry \n"))
